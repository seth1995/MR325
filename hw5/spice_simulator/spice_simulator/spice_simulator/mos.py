...
class NMOS(MOS):
    ...
    def Stamp(self,var):
        ...
        if (vgs>self.VTH ) and (vds<(vgs-self.VTH)) and vds>0:
            ...
            gds=2*kn*(vgs-self.VTH-vds)*(1+self.lam*vds)+self.lam*kn*(2*(vgs-self.VTH)*vds-vds**2)

            Ids=kn*(2*(vgs-self.VTH)*vds-vds**2)*(1+self.lam*vds)
...
class PMOS(MOS):
    ...
    def Stamp(self,var):
        ...
        if (vsg > self.VTH) and (vsd < (vsg - self.VTH)) and vsd>0:
            ...
            gds = 2 * kp * (vsg - self.VTH - vsd) * (1 + self.lam * vsd) + self.lam * kp * (
            2 * (vsg - self.VTH) * vsd - vsd ** 2)

            ISD = kp * (2 * (vsg - self.VTH) * vsd - vsd ** 2) * (1 + self.lam * vsd)
