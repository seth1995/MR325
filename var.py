...
class Var:
    ...
    def output_something(self):
        ...
        double_print(
            "****** HW6@MR325 2015-2016  (March 27 2016)  ******",
            self.fo)
    ...
    def initMartix(self):
        if self.ToSolveAC:
            self.G = np.zeros(
                [self.node_counter, self.node_counter], dtype=complex)
            self.B = np.zeros(
                [self.node_counter, self.exI_counter], dtype=complex)
            self.C = np.zeros(
                [self.exI_counter, self.node_counter], dtype=complex)
            self.I = np.zeros([self.exI_counter, 1], dtype=complex)
            self.U = np.zeros([self.node_counter, 1], dtype=complex)
            self.D = np.zeros(
                [self.exI_counter, self.exI_counter], dtype=complex)
        else:
    ...
    def solveDC(self, plotFlag):
        ...
        if not self.DC_target == "NONE":
            for CL in self.CL_set:
                # CL.tranInit(self)
                CL.Stamp(self)
            if self.DC_target not in self.Name_set:
